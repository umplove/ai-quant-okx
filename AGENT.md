...
- 项目用途：OKX 现货、杠杆、永续合约量化交易机器人，包含动量扫描、AI 决策/复盘、Telegram 控制、SQLite 持久化、经验评分和交易安全检查。
- 默认安全姿态：优先模拟盘；非模拟实盘必须显式设置 `ALLOW_LIVE_TRADING=true`。
- 交互和文档要求：默认使用中文回复用户；新增或更新项目文档时默认使用中文。公开文档保持通用，不写成只适合某个私人环境的说明。
...
## 当前安全架构
- `okx_quant_bot/momentum_runner.py` 负责动量主循环、OKX 状态同步、订单同步、仓位更新、硬退出、换仓和止损记录维护。
- `okx_quant_bot/exchange/okx.py` 负责 OKX REST 请求、client order id、instrument 精度校验、现货/杠杆/永续下单、撤单和止损算法单。
- `okx_quant_bot/data/storage.py` 负责 SQLite schema 迁移和订单、仓位、止损单、AI 决策、报告、经验评分、异常审计等持久化。
- SQLite `ALTER TABLE ... ADD COLUMN` 迁移只能使用常量默认值；时间字段先 nullable 添加，再回填。
...
- `MOMENTUM_ENTRY_MODE=rules_first` 下，规则先入场，AI 作为高置信风险否决、归因和真实模拟盘经验训练层，不再做影子市场空转训练。
...
- 真实执行支持现货、杠杆和永续。交割合约、期权、网格等不作为真实下单路径，除非用户明确要求实现。

## Git 和发布规则
- 重要：用户偏好是“改完就能让服务器直接拉到”。任何代码或文档修改完成并验证后，默认必须提交并推送到 GitHub。
- 默认发布目标为当前项目远端 `origin/main`，除非用户明确要求不推送、改用分支、改用 PR，或测试/编译失败。
- 即使开启新对话，也要遵守上一条：完成修改后不要只停留在本地工作区，要提交并推送。
...
## 后续协作注意
- PowerShell 终端可能把中文显示成乱码，但源文件按 UTF-8 保存并应能正常编译。
